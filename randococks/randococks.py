...
class randococks(commands.Cog):
  ...
  async def cock(self, ctx, *args):
    ...
    random.seed()
    await ctx.send(user.name + ' ' + self.middle_list[random.randint(0, len(self.middle_list) - 1)] + ' ' + length_string + ' inches of ' + self.ending_list[random.randint(0, len(self.ending_list) - 1)])
